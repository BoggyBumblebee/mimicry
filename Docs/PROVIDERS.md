...
- Mac App Store through `mas`: implemented for read-only App Store application inventory when `mas` is available through the app environment or standard Homebrew install locations. App inventory is captured as normal user-specific configuration; dry-run apply still requires user action for App Store installation because sign-in, purchase history, and app availability are user/account controlled.
...
Homebrew absence, `mas` absence, failed or timed-out `mas list` inventory, unreadable Finder preferences, unreadable Terminal files, Terminal files containing secret-like values, iCloud states requiring user action, and missing or unreadable Safari, Chrome, or Firefox bookmarks are represented as warnings or absent/redacted/reviewable values inside the relevant section. Environment metadata, including hardware model, hostname, and username, is retained for provenance and comparison only; apply planning skips the environment section as informational instead of turning those machine-specific values into work for another Mac. App Store app inventory is retained as user-specific captured configuration, while App Store installation remains user-action work. Snapshot, inspect, validate, diff, dry-run apply, and browser bookmark export do not install, remove, upgrade, sign in, write preferences, edit shell files, read synced document contents, import bookmarks into a browser profile, or change system settings.
