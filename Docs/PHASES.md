...
Status: In progress for the GUI. The first polish slice replaced the Phase 1 placeholder with a native SwiftUI shell, Snapshot/Apply/Compare/History/Diagnostics surfaces, a Settings scene, and app smoke/content tests. Phase 6A wires the app to the existing snapshot engine for package creation, local diagnostics refresh, and recent package history. Phase 6B opens existing `.mimicry` packages, validates package contents through the core package store, and surfaces source metadata, section summaries, warnings, and safety/review/excluded/unsupported counts in Snapshot and History. Phase 6C compares an open package with the current Mac through the existing diff engine and renders matching, changed, missing, current-only, skipped, and blocked groups. Phase 6D previews the existing non-mutating dry-run apply plan from the Apply tab, including install, configure, skip, blocked, and user-action groups. Phase 6E exports a reviewable browser bookmark HTML handoff from the Apply tab without touching browser profiles. Phase 6F runs the backed-up Finder-safe confirmed apply path from the Apply tab after dry-run review. Phase 6G records GUI workflow audit events and exports them as structured JSON from History. Phase 6H surfaces managed, machine-specific, hardware-specific, user-specific, and unsupported compatibility counts in package review/apply and adds management detail to diagnostics. Phase 6I makes package section items and warnings reviewable inside the app and hardens Homebrew capture for GUI launches by probing standard Homebrew executable paths. The latest Snapshot polish moves Create Snapshot and Open Package into the app toolbar plus Package menu, opens the app directly on Snapshot with workflow/provider guidance before a package is created or opened, and keeps the sidebar focused on user workflows rather than informational dashboard content.
